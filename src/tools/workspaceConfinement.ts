 ...
 *
 * By default devnull's file tools resolve any path handed to them — absolute paths are used
 ...
 *
 * Set DEVNULL_RESTRICT_TO_WORKSPACE=true to opt into confinement: any resolved path that would
...
export function isWorkspaceConfinementEnabled(): boolean {
  return process.env.DEVNULL_RESTRICT_TO_WORKSPACE === "true";
}
  ...
  constructor(filePath: string, cwd: string) {
    super(
      `Refused: "${filePath}" resolves outside the workspace root "${cwd}". ` +
        `Set DEVNULL_RESTRICT_TO_WORKSPACE=false (or unset it) to allow paths outside the workspace.`
    );
