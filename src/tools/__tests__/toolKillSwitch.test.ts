...
afterEach(() => {
  delete process.env.XCODER_DISABLE_SHELL_TOOLS;
...
describe("tool kill switches (security fix — no per-tenant sandboxing, so operators need a way to fully disable shell/network tools)", () => {
  it("XCODER_DISABLE_SHELL_TOOLS blocks run_command_tool at the dispatch layer", async () => {
    process.env.XCODER_DISABLE_SHELL_TOOLS = "1";
  ...
  it("XCODER_DISABLE_SHELL_TOOLS blocks every ssh_*/docker_* tool, not just run_command_tool", async () => {
    process.env.XCODER_DISABLE_SHELL_TOOLS = "1";
  ...
  it("XCODER_DISABLE_NETWORK_TOOLS blocks outbound-fetch tools independently of the shell switch", async () => {
  ...
  it("tools NOT in either dangerous group are unaffected by either kill switch", async () => {
    ...
    expect(JSON.stringify(result.observation)).not.toContain("XCODER_DISABLE");
  });
  ...
  it("with neither env var set, dangerous tools are NOT blocked by the kill switch (default: available)", async () => {
    ...
    expect(JSON.stringify(result.observation)).not.toContain("XCODER_DISABLE_SHELL_TOOLS is set");
  });

  it("'true' and '1' are both accepted truthy values for the kill switches", async () => {
    process.env.XCODER_DISABLE_SHELL_TOOLS = "true";
    const result = await dispatchToolCall(call("run_command_tool", { command: "echo hi" }), "/tmp");
    expect(result.isError).toBe(true);
  });
});
